...
class CheckCalibration(object):
    ...
    def exec(self):
        wait_for_input("Press enter to move to home")
        self.hydra_group.set_named_target("zeros")
        self.execute_plan(self.hydra_group)

        self.hydra_group.set_pose_reference_frame("hydra_base")

        wait_for_input("Press enter to move to first point ('x' to cancel)")
        pose1 = [0, 0.25, 0, 0, 0, -0.7071068, 0.7071068]
        pose2 = [0, -0.25, 0, 0, 0, 0.7071068, 0.7071068]
        ...
        wait_for_input("Press enter to move to first approach point ('x' to cancel)")
        pose1 = [0, 0.02, 0, 0, 0, -0.7071068, 0.7071068]
        pose2 = [0, -0.02, 0, 0, 0, 0.7071068, 0.7071068]
        print(f"Moving to points:\n{pose1}\n{pose2}")
        self.hydra_group.set_pose_target(pose1, "rob1_flange")
        self.hydra_group.set_pose_target(pose2, "rob2_flange")
        self.execute_plan(self.hydra_group)

        wait_for_input("Press enter to move to second approach point ('x' to cancel)")
        self.hydra_group.set_max_velocity_scaling_factor(0.005)
        pose1 = [0, 0.005, 0, 0, 0, -0.7071068, 0.7071068]
        pose2 = [0, -0.005, 0, 0, 0, 0.7071068, 0.7071068]
        print(f"Moving to points:\n{pose1}\n{pose2}")
        self.hydra_group.set_pose_target(pose1, "rob1_flange")
        self.hydra_group.set_pose_target(pose2, "rob2_flange")
        self.execute_plan(self.hydra_group)

        wait_for_input("Press enter to move to 2mm gap ('x' to cancel)")
        self.hydra_group.set_max_velocity_scaling_factor(0.001)
        pose1 = [0, 0.001, 0, 0, 0, -0.7071068, 0.7071068]
        pose2 = [0, -0.001, 0, 0, 0, 0.7071068, 0.7071068]
        print(f"Moving to points:\n{pose1}\n{pose2}")
        self.hydra_group.set_pose_target(pose1, "rob1_flange")
        self.hydra_group.set_pose_target(pose2, "rob2_flange")
        self.execute_plan(self.hydra_group)
